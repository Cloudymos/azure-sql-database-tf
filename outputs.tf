...
output "resource_group_location" {
  ...
  value       = var.primary_sql_server_location
}
...
output "secondary_sql_server_fqdn" {
  ...
  value       = element(concat(azurerm_mssql_server.primary.*.fully_qualified_domain_name, [""]), 0)
}
...
output "primary_sql_server_private_endpoint" {
  ...
  value       = azurerm_private_endpoint.pep1.id
}
...
output "primary_sql_server_private_endpoint_ip" {
  ...
  value       = azurerm_private_endpoint.pep1.private_service_connection[0].private_ip_address
}

output "primary_sql_server_private_endpoint_fqdn" {
  description = "Primary SQL server private endpoint FQDN "
  value       = data.azurerm_private_dns_a_record.sqlserver_a_record.fqdn
}